...
export async function POST(req: NextRequest) {
  ...
  page.drawRectangle({
    ...
    width: 842,
    height: 595,
    color: rgb(0.035, 0.031, 0.024)
  });

  page.drawText("Lumos Executive Intelligence", {
    x: 42,
    y: 540,
    size: 24,
    font: bold,
    color: rgb(0.83, 0.68, 0.22)
  });

  page.drawText(safeText(s.summary.headline, 110), {
    x: 42,
    y: 510,
    size: 11,
    font,
  ...
  content.forEach((text: string) => {
    ...
    if (text.trim().length > 0) {
      page.drawText(safeText(text, 135), {
        x: 48,
        y,
        size: isTitle ? 14 : 10,
  ...
  return new Response(body, {
    headers: {
      ...
      "content-disposition": "attachment; filename=lumos-intelligence.pdf"
    }
  });
}
